...
ARGS=$(getopt -o "h" -l "html,pdf,gfm,plain,tex,texi,output-dir:,help" -n "$0" -- "$@")
# echo $ARGS
...
if [ "x${format}" = "x.html" ]; then
  ...
  format_args="--pdf-engine=xelatex --listings --metadata=listings:true -H ${rootdir}/doc/syntax.tex ${rootdir}/doc/${pdfyaml}"
#   template_args="--template=${rootdir}/doc/${template}.tex"
  ...
  format_args="--pdf-engine=xelatex --listings --metadata=listings:true -H ${rootdir}/doc/crossref.tex -H ${rootdir}/doc/syntax.tex"
#   template_args="--template=${rootdir}/doc/${template}.tex"

elif [ "x${format}" = "x.texi" ]; then
 
  format_args="--reference-links --reference-location=section -t texinfo"
  template_args="--template=${rootdir}/doc/${template}.texi"
